...
??? Note "解题思路"
    ...
    设站在最上面的巨人编号为 $x$，则此时答案为 $\sum\limits_{i = 1} ^ N A_i - A_x + B_x$，而 $\sum\limits_{i = 1} ^ N A_i$ 是定制，因此找出 $B_x - A_x$ 的最大值即可。
...
???+ Abstract "题目大意"
    ...
    接下来，你需要执行 $M(1 \le M \le 2 \times 10^5)$ 轮加边点操作，每一轮操作给你两个整数 $K_i$ 和 $C_i$ 以及一个点的集合 $S_i = \{A_{i, 1}, A_{i, 2}, \cdots, A_{i, K_i}\}$，你需要给集合 $S_i$ 中的任意两点都添加一条权值为 $C_i$ 的边。
...
??? Note "解题思路"
    ...
    因此我们在枚举的时候，跳过所有的孤立点，同时统计孤立点的数量，对于一个合法的方案，将其记录到每个点点可能的取值当中（用 `set` 存储）。
